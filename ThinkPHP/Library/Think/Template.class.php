...
class  Template {
    ...
    public function loadTemplate ($tmplTemplateFile,$prefix='') {
        ...
        // 修改以支持http/https
        if(is_file($tmplTemplateFile)
         || strpos($tmplTemplateFile, 'http://') === 0
         || strpos($tmplTemplateFile, 'https://') === 0) {
    ...
    private function parseTemplateName($templateName){
        ...
        foreach ($array as $templateName){
            ...
            {
                // 以模板目录作为根目录，使用相对路径或绝对路径
                if (substr($templateName, 0, 1) === '/')
                {
                    $templateName = THEME_PATH.$templateName;
                }
                else
                {
                    // 相对路径，同时支持http/https
                    if (strpos($this->templateFile, 'http://') === 0
                     || strpos($this->templateFile, 'https://') === 0)
                    {
                        $templateName = $this->rel2abs($templateName, $this->templateFile);
                    }
                    else
                    {
                        $templateName = dirname($this->templateFile).'/'.$templateName;
                    }
                }
            }
            // 获取模板文件内容
